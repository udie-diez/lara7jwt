...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('laporanPerubahanmodal') }}">
            @csrf
        ...
        <table class="table table-hover tlaporan" id="table_1">
            ...
            <tbody>
                ...
                foreach ($datasaldo as $saldoawal) {      //saldo awal periode filter
                    ...
                    if (substr($saldoawal->kode, 0, 2) == '30') {
                        $modal += $saldoawal->saldo;
                    }
                }
                ...
                if ($row->kode == '30.0000') { //modal
                    foreach ($data as $h1) {
                        if ((substr($h1->kode, 0, 2) == '30') && $h1->jenis == 1) {
                            $modal += abs($h1->saldo);
                        }
                    };
                }

                if (substr($row->kode, -4) == '0000' && substr($row->kode, 0, 2) == '30') {
                    foreach ($datasaldo as $saldoawal) {      //saldo awal periode filter
                ...
                } else if (substr($row->kode, -3) == '000' && substr($row->kode, 0, 2) == '30' && $row->jenis != 1) {
                    foreach ($datasaldo as $saldoawal) {      //saldo awal periode filter
                ...
                <tr class='font-weight-bold foot1'>
                    <td>MODAL AKHIR</td>
                    <td class='text-right'>{{ Rupiah($modal + $labakotor - $biaya + $pendapatan - $pengeluaran , 2) }}</td>
                </tr>
