...
<div class="card">
		...
		<div class="header-elements">
			...
			<div class="btn-group">
				...
				<div class="dropdown-menu dropdown-menu-right">
					<a href="{{ action('PembelianPenawaranCont@createPemesanan', $data->id ?? 0) }}" class="dropdown-item" title="Buat Pemesanan"> Buat Pemesanan</a>
					<a href="{{ action('PembelianPenawaranCont@cetak', $data->id ?? 0) }}" class="dropdown-item" title="Cetak Penawaran"> Cetak Penawaran</a>
				</div>
			</div>
		</div>
	</div>
	<div class="card-body">
		<div class="row">
			<div class="col-sm-12">
				<form method="POST" action="{{ route('update_pq') }}">
					<fieldset>
						...
						<div class="form-group row">
							...
							<div class="col-sm-2">
								<input type="text" name="tanggal" placeholder="Tanggal" class="form-control pickadate" value="{{ IndoTgl($data->tanggal ?? old('tanggal')) }}">
							</div>
						</div>
								...
								<table id="tabel-item" class="table" width='100%'>
									...
									<tbody id="bbody">
										...
										?>
											...
											<td><a href="{{ action('PembelianPenawaranCont@destroyProduk',['id'=>$row->id]) }}" class="list-icons-item text-danger-600" title="Hapus Data" onclick="return confirm('Anda yakin ingin menghapus data ini ??')"><i class="icon-bin"></i></a></td>
											@endif
											</tr>
										<?php
										}
						...
						<div class="form-group row mt-1">
							<div class="col-sm-11">
								...
								<div class="list-icons" style="float: right;">
									<a type="button" href="{{ $kodeubah==1 ? route('show_pq',$data->id ?? '' ) : route('pq') }}" class="btn btn-outline-success btn-sm" title="Batal / Kembali">{{ $kodeubah==1 ? 'BATAL' : 'KEMBALI' }}</a>
									...
									@else
									<a type="button" href="{{ route('edit_pq',$data->id ?? '') }}" class="btn btn-outline-info btn-sm" title="Ubah Data">Ubah</a>
									@endif
								</div>
							</div>
						</div>
