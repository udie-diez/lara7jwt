...
class ProjectCont extends Controller
{
    ...
    public function index()
    {
        ...
        (SELECT sum(pb.nilai) as bayar from project pp left join invoice i on pp.id = i.projectid left join pembayaran pb on pb.invoiceid = i.id where pp.id = p.id) as pembayaran
        from project p inner join perusahaan on p.perusahaanid = perusahaan.id left join project_pemesan pp on pp.id = p.pemesanid left join pengelola on pengelola.id = p.pic order by p.id DESC limit 0,300');
    ...
    public function filterInvoice(Request $request)
    {
        ...
        $q = 'SELECT i.*, p.nama,pp.nama as pemesan, p.no_po, p.no_spk, perusahaan.alias as perusahaan, perusahaan.unitkerja, pengelola.nama as pic,  (i.total - i.ppn) as subtotal, i.ppn as pajak
    ...
    public function updateItem(Request $request)
    {
        ...
        $cbkeuntungan = $request->cb_keuntungan;
        ...
        for ($i = 0; $i < $jmlbaris; $i++) {
            ...
            if (isset($iditem[$i]) && $iditem[$i] > 0) {
                $update = ProjectItem::find($iditem[$i]);
                $update->update($arr);

                // $update = ProjectItem::where('id', $iditem[$i])->update($arr);
            } else {
                $insert = ProjectItem::create($arr);
            }
        ...
        if ($cbkeuntungan == 'true') {

            $keuntungan = Project::find($projectid)->keuntungan;
        ...
        if ($update->tgl_spk < "2022-04-01") {
            $ppnpersen = 10;
            $nilaippn = $subtotal * 0.1;
        } else {
            $ppnpersen = 11;
            $nilaippn = $subtotal * (MyHelpers::nilaiPpn() / 100);
        }

        $total = $subtotal + $nilaippn;

        $update->update(['nilai' => $total, 'ppnnilai' => $nilaippn, 'ppnpersen' => $ppnpersen, 'cbkeuntungan' => ($cbkeuntungan == 'true' ? 1 : 0)]);
    ...
    public function invoice()
    {
        ...
        $nilaippn = MyHelpers::nilaiPpn() / 100;

        if (Auth::user()->role == 'admin') {
            $limit = 'limit 0,300';
            $data = DB::select('SELECT i.*, p.nama, pp.nama as pemesan , p.no_po, p.no_spk, perusahaan.alias as perusahaan,perusahaan.unitkerja, pengelola.nama as pic, i.ppn as pajak, (i.total - i.ppn) as subtotal 
            from invoice i inner join project p on i.projectid = p.id inner join perusahaan on p.perusahaanid = perusahaan.id left join pengelola on pengelola.id = p.pic left join project_pemesan pp on pp.id = p.pemesanid where i.jenis = 1 and i.status = 3 order by p.id DESC ' . $limit);
        } else {
            $limit = 'limit 0,300';

            $data = DB::select('SELECT i.*, p.nama, pp.nama as pemesan , p.no_po, p.no_spk, perusahaan.alias as perusahaan,perusahaan.unitkerja, pengelola.nama as pic, i.ppn as pajak, (i.total - i.ppn) as subtotal 
            from invoice i inner join project p on i.projectid = p.id inner join perusahaan on p.perusahaanid = perusahaan.id left join pengelola on pengelola.id = p.pic left join project_pemesan pp on pp.id = p.pemesanid where i.jenis = 1 order by p.id DESC ' . $limit);
        }
    ...
    public function panjar()
    {
        ...
        $data = Panjar::leftjoin('project', 'project.id', 'panjar.projectid')->leftjoin('perusahaan', 'project.perusahaanid', 'perusahaan.id')
            ...
            // ->where('panjar.jenis',0)
            ->select('panjar.*', 'panjar.id as panjarid', 'panjar.nilai as nilaipanjar', 'project.*', 'project.nama as uraian', 'perusahaan.nama as perusahaan', 'project_pemesan.nama as penerima', 'panjar.status as statuspanjar',  'pembayaran.tanggal as tanggalbayar', DB::raw('(select sum(pd.nilai) from panjar_detail pd inner join project  p on pd.projectid = p.id   where panjarid = panjar.id group by pd.panjarid) as jumlahpenggunaan'))
            ->get();
        $tag = ['menu' => 'Panjar', 'submenu' => 'Daftar Panjar', 'judul' => 'DAFTAR PANJAR', 'menuurl' => 'panjar', 'modal' => 'true'];
        $kodeubah = 3;
    ...
    public function createPanjar()
    {
        ...
        $tag = ['menu' => 'Panjar', 'submenu' => 'Input Panjar', 'judul' => 'INPUT PANJAR', 'menuurl' => 'panjar', 'modal' => 'true'];
        $pengelola = Pengelola::where('status', 1)->get();
    ...
    public function infoProject(Request $request)
    {
        ...
        $tag = ['menu' => 'Project', 'submenu' => 'Input Invoice', 'judul' => 'INPUT INVOICE', 'menuurl' => 'invoice', 'modal' => 'true'];
        return view('project.createInvoice', compact('tag', 'data', 'projectid_f', 'project', 'item', 'pengurus', 'invoice', 'jenisinvoice', 'bayarinvoice', 'kodeubah', 'potongan', 'daftarpembayaran', 'nomor', 'nilaippn'));
    }
    ...
    public function updateInvoice(Request $request)
    {
        ...
        if ($this->EnglishTgl($request->tanggal) >= "2022-04-01" and $project->ppnpersen == 10) {
            $subtotal = $project->nilai - $project->ppnnilai;
            $ppnnilai = $subtotal * (MyHelpers::nilaiPpn() / 100);
            $total = $subtotal + $ppnnilai;
            $project->nilai = $total;
            $project->ppnnilai = $ppnnilai;
            $project->ppnpersen = MyHelpers::nilaiPpn();
            $project->save();
        } else {
            $total = $request->total;
            $ppnnilai = $request->ppn;
        }
        ...
        if ($invid > 0) {
            ...
            MyHelpers::inputTransaksi($this->EnglishTgl($request->tanggal), 'penjualan', $invid, $akunpiutang, $total, 0, 1);  //piutang
            MyHelpers::inputTransaksi($this->EnglishTgl($request->tanggal), 'penjualan', $invid, $akunppn, 0, $ppn, 0);  //ppn
            MyHelpers::inputTransaksi($this->EnglishTgl($request->tanggal), 'penjualan', $invid, $akunpenjualan, 0, $nilai, 0);  //penjualan
            if ($pph22 > 0) {
    ...
    public function editPanjar($id, $kodeubah = 1, $kode = 0)
    {
        ...
        $tag = ['menu' => 'Panjar', 'submenu' => 'Data Panjar', 'judul' => 'DATA PANJAR', 'menuurl' => 'panjar', 'modal' => 'true'];
        return view('project.createPanjar', compact('tag', 'data', 'projectid_f',  'project', 'kodeubah', 'item', 'pengelola', 'panjar', 'pemesan', 'daftarpembayaran', 'panjardetail', 'pengurus'));
    }
    ...
    public function editInvoice($id, $kodeubah = 1, $kode = 0)
    {
        ...
        $data = Invoice::join('project', 'project.id', 'invoice.projectid')
            ...
            ->where('invoice.id', $id)
            ->select('project.*', 'perusahaan.nama as perusahaan', 'perusahaan.alias', 'perusahaan.unitkerja')
            ->first();
        ...
        return view('project.createInvoice', compact('tag', 'data', 'item', 'pengurus', 'invoice', 'jenisinvoice', 'bayarinvoice', 'kodeubah', 'bayarinvoice', 'daftarpembayaran', 'nomor', 'nilaippn'));
    }
...
(SELECT sum(((pm.porsi/100) * p.nilai)) as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id where p.status = 1 AND year(p.tgl_spk) = " . $tahun . " AND pm.pengelolaid = pl.id group by pm.pengelolaid) as tagihan,
(SELECT sum(((pm.porsi/100) * p.nilai)) as realisasi from projectAM pm inner join project p on pm.projectid = p.id left join invoice i on i.projectid = p.id where i.id is null AND p.status = 1 AND year(p.tgl_spk) = " . $tahun . " AND pm.pengelolaid = pl.id group by pm.pengelolaid) as potensi 
 from pengelola pl inner join target t on pl.id = t.pengelolaid and t.tahun = " . $tahun . " ; 
");

$progresbulan = DB::select("SELECT pl.id,pl.nama, t.nilai/1000000 as ttarget, (t.nilai/12)/ 1000000  as btarget,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 1 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b1,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000  as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 2 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b2,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 3 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b3,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 4 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b4,
        (SELECT sum(((pm.porsi/100) * p.nilai))/ 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 5 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b5,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 6 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b6,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 7 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b7,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 8 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b8,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 9 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b9,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 10 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b10,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 11 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b11,
        (SELECT sum(((pm.porsi/100) * p.nilai)) / 1000000 as realisasi from projectAM pm inner join project p on pm.projectid = p.id inner join invoice i on i.projectid = p.id  where p.status = 1 AND year(i.tanggal) = t.tahun AND month(i.tanggal) = 12 AND pm.pengelolaid = pl.id  group by pm.pengelolaid) as b12
         from pengelola pl inner join target t on pl.id = t.pengelolaid and t.tahun = ".$tahun." 
...
;");
        ...
        $breal = json_encode($breal);
        $btarget = json_encode($btarget);
        return view('project.dashboard', compact('tag', 'pengelola', 'breal', 'btarget', 'targettahun', 'realtahun', 'persenprogress', 'progrespegawai', 'rekap', 'progresbulan'));
    }
